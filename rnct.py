...
""" 
RinoZero V2.0
Creada el dia: 10-03-2018, 19:54
...
class Evaluador():
    ...
    def __init__(self,neuronasXcapa,funcionActivacion):
        self.neuronasXcapa = neuronasXcapa
        self.funcionActivacion = funcionActivacion
    ...
    def cargar_pesos(self,nombreRaizArchivo):
        
        pesos = [np.zeros((self.neuronasXcapa[i]+1,self.neuronasXcapa[i+1])) for i in range(len(self.neuronasXcapa)-1)]
        lista = [[] for i in range(len(self.neuronasXcapa)-1)]        
        
        for i in range(len(self.neuronasXcapa)-1):
            indice = 0
            for linea in open(nombreRaizArchivo+str(i)+'.csv','r').readlines():
                lista[i].append(linea.split(';'))

                del lista[i][indice][self.neuronasXcapa[i+1]]
                indice += 1
        
            for j in range(pesos[i].shape[0]):
                for k in range(pesos[i].shape[1]):
                    pesos[i][j][k] = float(lista[i][j][k])
    ...
    def clasificar(self,pesos,x_prueba,y_prueba):

        u = [0 for i in range(len(self.neuronasXcapa))]
        u[0] = x_prueba
        
        for i in range(1,len(u)):
            if self.funcionActivacion=="RELU":
                
                if i != len(u)-1:
                    u[i] = funcionRELU(u[i-1],pesos[i-1])
                
                else:
                    u[i] = funcionSigmoide(u[i-1],pesos[i-1]) 
